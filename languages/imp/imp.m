...
@rule/simple [bexpr.eq]: eq[X, Y] => z.eq(X, Y) where bool.and(z.is(X), z.is(Y)) ;
@rule/simple [bexpr.le]: le[X, Y] => z.le(X, Y) where bool.and(z.is(X), z.is(Y)) ;
@rule/simple [bexpr.lt]: lt[X, Y] => z.lt(X, Y) where bool.and(z.is(X), z.is(Y)) ;
@rule/simple [bexpr.neg]: not[X] => bool.neg(X) where bool.is(X) ;

@rule/simple [stmt.ite.true]: ite[B, X, Y] => X where bool.and(bool.is(B), bool.eq(B, bool.true())) ;
@rule/simple [stmt.ite.false]: ite[B, X, Y] => Y where bool.and(bool.is(B), bool.eq(B, bool.false())) ;

@rule/simple [while.unfold]: while[B, S] => ite[B, seq[S, while[B, S]], unitValue[]] where bool.true() ;
