...
Lemma ipmap_lookup
    ...
    l !! idx = Some a ->
    ∃ (b : B), 
        (ipmap l f) !! idx = Some b /\
        forall pf,
            b = f idx a pf
.
Proof.
    intros Hlidx.
    destruct ((ipmap l f) !! idx) eqn:Heq.
    {
        exists b.
        split>[reflexivity|].
        intros pf.
        ...
        destruct ((pfseqn (length l) !! idx)) eqn:Heq2.
        {
            simpl in Heq2.
            ...
            destruct s as [x pfx].
            simpl in *.
            rewrite <- (pflookup_spec _ _ pfx) in Hlidx.
            apply (inj Some) in Hlidx.
            remember (f x) as fx.
            ltac1:(move: (eq_sym _)).
            revert pf.
            rewrite Hlidx.
    ...
    }
    {
        ...
        rewrite length_fmap in Heq.
        rewrite length_pfseqn in Heq.
        apply lookup_lt_Some in Hlidx.
        ltac1:(lia).
    }
Qed.
...
#[global]
Instance map_same_up_to_sym
    {K V : Type}
    {_ : EqDecision K}
    {_ : Countable K}
    (base : gset K)
    :
    Symmetric (@map_same_up_to K V _ _ base)
.
Proof.
    unfold Symmetric.
    intros x y H.
    unfold map_same_up_to.
    unfold map_same_up_to in H.
    destruct H as [H1 H2].
    split.
    { symmetry. apply H1. }
    {
...
Proof.
    ...
    }
    {
        ...
        unfold fmap in IHl.
        rewrite <- IHl.
        do 3 (rewrite lookup_difference).
