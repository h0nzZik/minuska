...
Lemma make_parallel0_map_to_list
    ...
    l ≡ₚ map_to_list s ->
    make_parallel0 init l = subp_compose init s
.
Proof.
    revert init l.
    ltac1:(induction s using map_ind); intros init l Hsnorm Hinit Hinit2 Hinit3 Hinit4 Hinit5 Hl.
    {
        unfold make_parallel.
        unfold SubP in *.
        unfold make_parallel0.
        rewrite map_to_list_empty in Hl.
        symmetry in Hl.
        apply Permutation_nil in Hl.
        subst l.
        simpl.
                                                        ...
                                                        ).
                                                        {
                                                            ...
                                                            ltac1:(apply map_filter_strong_ext).
                                                            {
                                                                ...
                                                                split.
                                                                {
                                                                    ...
                                                                    {
                                                                        apply elem_of_dom_2 in H1z as H1z''.
                                                                        eapply elem_of_weaken in H1z'' as H1z'''>[|apply dom_subp_compose_subseteq].
                                                                        (* Search dom subp_compose. *)
...
Proof.
    intros H1 H2 Hl.
    unfold make_parallel.
    rewrite make_parallel0_map_to_list with (s := s).
    {
        rewrite subp_compose_empty_l.
        { reflexivity. }
        { exact H1. }
...
:
    dom m ## subp_codom m ->
    subp_is_normal m ->
    subp_codom (rlift (r_inverse (renaming_for avoid0 m)))
...
:
    dom m ## subp_codom m ->
    subp_is_normal m ->
    subp_codom (rlift ((renaming_for avoid0 m)))
        ## subp_dom (rlift ((renaming_for avoid0 m)))
.
Proof.
    intros H1 H2.
    assert (Htmp := another_helper_lemma avoid0 m H1 H2).
    rewrite subp_codom_inverse in Htmp.
    rewrite subp_dom_inverse in Htmp.
    symmetry.
    exact Htmp.
...
Proof.
    ...
    assert(Hhelp: subp_codom (rlift (r_inverse (renaming_for avoid0 m)))
        ## subp_dom (rlift (r_inverse (renaming_for avoid0 m)))).
    {
        apply another_helper_lemma.
        apply Hdocodo.
        apply Hnorm.
    }
    assert(Hhelp2: subp_codom (rlift (renaming_for avoid0 m)) ## subp_dom (rlift (renaming_for avoid0 m))).
    {
        assert (Htmp := subp_codom_renaming_for_disjoint_dom_m avoid0 m).
        unfold subp_dom, SubP.
        unfold rlift at 2.
        rewrite dom_fmap.
        ltac1:(rewrite dom_renaming_for).
        ltac1:(set_solver).
    }

    assert(Hhelp3: subp_dom (rlift (renaming_for avoid0 m)) ## subp_codom (rlift (renaming_for avoid0 m))).
    {    
        assert (Htmp := subp_codom_renaming_for_disjoint_dom_m avoid0 m).
        unfold subp_dom, SubP.
        unfold rlift at 1.
    ...
    rewrite make_parallel_app.
    {
        rewrite make_parallel_app.
        {
            ltac1:(rewrite (make_parallel_map_to_list (rlift _))).
            {
    ...
    {
        assert (H: subp_is_normal ((rlift (r_inverse (renaming_for avoid0 m))))).
        {
            apply inverse_of_renaming_is_normal.
        }
        rewrite subp_is_normal_spec in H.
    ...
    }
    {
        ...
        {
            assert (Htmp2 := subp_codom_subp_compose_2 m (rlift (r_inverse (renaming_for avoid0 m)))).
            assert (Htmp3 := dom_subp_compose_subseteq m (rlift (r_inverse (renaming_for avoid0 m)))).
            ...
            rewrite subp_codom_inverse in Hhelp>[|apply renaming_for_ok].
            
            rewrite subp_codom_inverse in Htmp2>[|apply renaming_for_ok].
            rewrite subp_dom_inverse in Hhelp>[|apply renaming_for_ok].
            assert(Htmp5 := subp_dom_inverse (renaming_for avoid0 m)).
            ltac1:(ospecialize (Htmp5 _)).
            {
                apply renaming_for_ok.
            }
            unfold subp_dom in Htmp5.
            ltac1:(rewrite Htmp5 in Htmp3).
            clear Htmp5.
            ...
            unfold rlift in Htmp4 at 2.
            rewrite dom_fmap in Htmp4.
            rewrite dom_renaming_for in Htmp4.
            unfold rlift in Htmp2 at 2.
            rewrite dom_fmap in Htmp2.
            rewrite dom_renaming_for in Htmp2.
            unfold subp_dom,subp_dom, SubP in *.
            (* rewrite dom_rlift_inverse in Htmp5. *)
            remember (subp_codom (subp_compose m (rlift (r_inverse (renaming_for avoid0 m))))) as A.
            remember (dom (subp_compose m (rlift (r_inverse (renaming_for avoid0 m))))) as B.

            (* Search dom renaming_for. *)
            (* remember (dom (rlift (renaming_for avoid0 m))) as A'. *)
            (* remember (subp_codom (rlift (renaming_for avoid0 m))) as B'. *)
            (* remember ((rlift (renaming_for avoid0 m))) as R. *)
            (* ltac1:(rewrite dom_rlift_inverse in Htmp5). *)
            (* ltac1:(rewrite <- HeqB' in Htmp2). *)
            (* ltac1:(set_solver). *)
        }
    }
    {
...
Lemma make_serial1_correct
    ...
    dom m ## subp_codom m ->
    vars_of φ ⊆ avoid ->
    subs_app (make_serial1 m avoid) φ = subp_app m φ
.
Proof.
    intros Hnl Havoid.
    unfold make_serial1.
    rewrite subs_app_app'.
    revert m avoid Hnl Havoid.
    induction φ; intros m avoid Hnl Havoid.
    {
        simpl.
        destruct a; simpl in *.
        {
            ...
            {    
                rewrite subs_app_nodup_1.
                {
                    ...
                    { reflexivity. }
                    {
                        ...
                        rewrite elem_of_list_to_set.
                        intros HC.
                        unfold srlift, sr_inverse in HC.
                        rewrite <- list_fmap_compose in HC.
                        rewrite elem_of_list_fmap in HC.
                        destruct HC as [[y1 y2][H1y H2y]].

                        (* subst. *)
                        rewrite elem_of_reverse in H2y.
                        unfold pair_swap in H2y.
                        rewrite elem_of_list_fmap in H2y.
                        destruct H2y as [[z1 z2][H1z H2z]].
                        simpl in *.
                        ltac1:(simplify_eq/=).
                        rewrite elem_of_map_to_list in H2z.
                        assert (Htmp := renaming_for_avoid avoid m).
                        (* Set Printing Implicit. *)
                        assert (Htmp2: z2 ∈ (@map_img _ _ _ _ (gset _) _ _ _ (renaming_for avoid m))).
                        {
                            rewrite elem_of_map_img.
                            exists z1.
                            exact H2z.
                ...
                }
                {
                    intros HContra.
                    rewrite elem_of_list_fmap in HContra.
                    destruct HContra as [[y p][H1 H2]].
                    ltac1:(simplify_eq/=).
                    unfold subs_precomposes in H2.
                    rewrite elem_of_list_fmap in H2.
                    destruct H2 as [[z q][H3 H4]].
                    ltac1:(simplify_eq/=).
                    rewrite elem_of_map_to_list in H4.
                    ltac1:(rewrite H in H4).
                    inversion H4.
                }
            }
        }
    }
    {
        ...
        rewrite subs_app_term.
        f_equal.
        rewrite vars_of_t_term in Havoid.
        clear s.
        revert avoid m Hnl Havoid H.
        induction l;
            intros avoid m Hnl Havoid H.
        {
            simpl.
            reflexivity.
        }
        {
            ...
            specialize (IHl avoid m Hnl ltac:(set_solver) H2).
            f_equal.
            {
                apply H1.
                { assumption. }
...
Lemma subs_is_normal_make_serial1
    ...
    subp_is_normal m ->
    dom m ## subp_codom m ->
    subs_is_normal (make_serial1 m avoid)
.
Proof.
    intros HH1 HH2.
    unfold subs_is_normal.
    ...
    intros [z p] H1z H2z.
    rewrite elem_of_app in H1z.
    destruct H1z as [H1|H1].
    {
        unfold subs_precomposes in H1.
        rewrite elem_of_list_fmap in H1.
        destruct H1 as [z' [H1z' H2z']].
        ltac1:(simplify_eq/=).
        destruct z' as [z1 z2].
        rewrite elem_of_map_to_list in H2z'.
        simpl in *.
        rewrite <- (reverse_involutive (srlift _)) in H.
        rewrite <- make_parallel_correct in H.
        rewrite make_parallel_map_to_list with (s := rlift (renaming_for avoid m)) in H.
        {
            apply helper_lemma' in H.
            ...
            destruct H2y as [H2y|H2y].
            {
                ...
                unfold rlift in H2y.
                unfold SubP in *.
                rewrite lookup_fmap in H2y.
                rewrite fmap_Some in H2y.
                destruct H2y as [x [H1x H2x]].
                ltac1:(simplify_eq/=).
                unfold renaming_for in H1x.
                rewrite <- elem_of_list_to_map in H1x.
                {
                    apply elem_of_zip_l in H1x as Hy.
                    apply elem_of_zip_r in H1x as Hx.
                    clear H1x.
                    rewrite elem_of_elements in Hy.
                    unfold SubP in *.
                    rewrite elem_of_dom in Hy.
                    destruct Hy as [z Hz].
            ...
            {
                subst.
                rewrite subp_is_normal_spec in HH1.
                ltac1:(naive_solver).
            }
        }
        {
    ...
    }
    {
        ...
        destruct H1 as [[z1 z2][H1z H2z]].
        ltac1:(simplify_eq/=).
        unfold sr_inverse in H2z.
        rewrite elem_of_reverse in H2z.
        rewrite elem_of_list_fmap in H2z.
        destruct  H2z as [[y p] [H1 H2]].
        unfold pair_swap in H1.
        ltac1:(simplify_eq/=).
        rewrite elem_of_map_to_list in H2.
        unfold renaming_for in H2.
        rewrite <- elem_of_list_to_map in H2.
        {
            apply elem_of_zip_l in H2 as H3.
            apply elem_of_zip_r in H2 as H4.
