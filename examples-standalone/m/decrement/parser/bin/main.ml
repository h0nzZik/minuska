...
let rec print_term (t : Syntax.term) =
  match t with
  | `Zero () -> "zero[]"
  | `Succ x -> "succ[" ^ (print_term x)  ^ "]"

let print_ast ast oux =
    let s = print_term ast in
    fprintf oux "%s" s;
    ()
...
let command =
  Command.basic
    ~summary:"Generate a Minuska-compatible parser for the Decrement programming language"
    ~readme:(fun () -> "TODO")
    (let%map_open.Command
        input_filename = anon (("program.dec" %: Filename_unix.arg_type)) and
